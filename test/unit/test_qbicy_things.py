...
def test_valid_qbic_address():
    from qbiclib import is_valid_qbic_address

    main = valid_qbic_address()
    test = valid_qbic_address('testnet')

    assert is_valid_qbic_address(main) is True
    assert is_valid_qbic_address(main, 'mainnet') is True
    assert is_valid_qbic_address(main, 'testnet') is False

    assert is_valid_qbic_address(test) is False
    assert is_valid_qbic_address(test, 'mainnet') is False
    assert is_valid_qbic_address(test, 'testnet') is True


def test_invalid_qbic_address():
    from qbiclib import is_valid_qbic_address

    main = invalid_qbic_address()
    test = invalid_qbic_address('testnet')

    assert is_valid_qbic_address(main) is False
    assert is_valid_qbic_address(main, 'mainnet') is False
    assert is_valid_qbic_address(main, 'testnet') is False

    assert is_valid_qbic_address(test) is False
    assert is_valid_qbic_address(test, 'mainnet') is False
    assert is_valid_qbic_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from qbiclib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % qbiclib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(qbiclib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(qbiclib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(qbiclib.blocks_to_seconds(16616)) == 2612035
