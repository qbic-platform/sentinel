...
def has_qbic_conf():
    ...
    valid_qbic_conf = False

    # ensure qbic_conf exists & readable
    #
    # if not, print a message stating that Qbic Core must be installed and
    # configured, including JSONRPC access in qbic.conf
    try:
        f = io.open(config.qbic_conf)
        valid_qbic_conf = True
    except IOError as e:
        print(e)

    return valid_qbic_conf
...
def main():
    ...
    if not has_qbic_conf():
        print("QbicCore must be installed and configured, including JSONRPC access in qbic.conf")
        sys.exit(1)
