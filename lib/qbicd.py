...
class QbicDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some qbicd methods
    ...
    @classmethod
    def from_qbic_conf(self, qbic_dot_conf):
        from qbic_config import QbicConfig
        config_text = QbicConfig.slurp_config_file(qbic_dot_conf)
        creds = QbicConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = qbiclib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = qbiclib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = qbiclib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_qbicd_proto_version_with_sentinel_ping)
