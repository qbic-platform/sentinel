...
def is_valid_qbic_address(address, network='mainnet'):
    ...
    qbic_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Qbic address
        return False

    if (address_version != qbic_version):
        return False
...
# shims 'til we can fix the qbicd side
def SHIM_serialise_for_qbicd(sentinel_hex):
    from models import QBICD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = QBICD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in qbicd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # qbicd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    qbicd_hex = serialise(obj)
    return qbicd_hex


# shims 'til we can fix the qbicd side
def SHIM_deserialise_from_qbicd(qbicd_hex):
    from models import QBICD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(qbicd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('qbic.conf')
