...
class GovernanceObject(BaseModel):
    ...
    # sync qbicd gobject list with our local relational DB backend
    @classmethod
    def sync(self, qbicd):
        golist = qbicd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_qbicd(qbicd, item)
    ...
    @classmethod
    def import_gobject_from_qbicd(self, qbicd, rec):
        import decimal
        import qbiclib
        import inflection
        ...
        # shim/qbicd conversion
        object_hex = qbiclib.SHIM_deserialise_from_qbicd(object_hex)
        objects = qbiclib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from qbicd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from qbicd, with every run
        ...
        # get/create, then sync payment amounts, etc. from qbicd - Qbicd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(qbicd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from qbicd! %s" % e)
            govobj.vote_delete(qbicd)
            return (govobj, None)
    ...
    def vote_delete(self, qbicd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(qbicd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, qbicd, signal, outcome):
        import qbiclib
        ...
        output = qbicd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = qbiclib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(qbicd, signal)

    def sync_network_vote(self, qbicd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = qbicd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = QBICD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 qbic addr, non-multisig
            if not qbiclib.is_valid_qbic_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Qbic address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (qbiclib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/QbicDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import qbiclib
        obj_data = qbiclib.SHIM_serialise_for_qbicd(self.serialise())
    ...
    def prepare(self, qbicd):
        try:
            object_hash = qbicd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = QBICD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not qbiclib.is_valid_qbic_address(addr, config.network):
    ...
    def hash(self):
        import qbiclib
        return qbiclib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = QBICD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, qbicd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - qbicd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, qbicd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - qbicd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, qbicd):
        now = int(time.time())
        return (self.created_at < (now - qbicd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, qbicd):
        if self.is_expired(qbicd):
            return False

        return True

    def is_deletable(self, qbicd):
        if self.is_expired(qbicd):
            return True
