...
class QbicConfig():
    ...
    def slurp_config_file(self, filename):
        # read qbic.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is qbic.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from qbic.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Qbic defaults...
        default_port = 17196 if (network == 'mainnet') else 18196

        # use default port for network if not specified in qbic.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from qbic.conf
