...
sentinel_cfg = QbicConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_qbicd_proto_version_with_sentinel_ping = 70207


def get_qbic_conf():
    home = os.environ.get('HOME')

    qbic_conf = os.path.join(home, ".qbiccore/qbic.conf")
    if sys.platform == 'darwin':
        qbic_conf = os.path.join(home, "Library/Application Support/QbicCore/qbic.conf")

    qbic_conf = sentinel_cfg.get('qbic_conf', qbic_conf)
